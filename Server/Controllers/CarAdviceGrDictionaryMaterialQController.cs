...
namespace SCMDWH.Server.Controllers
{
    ...
    [ApiController]
    public class CarAdviceGrDictionaryMaterialQController : ControllerBase
    {

        private readonly PurchasingContext _context;

        public CarAdviceGrDictionaryMaterialQController(PurchasingContext context)
        {
            _context = context;
        }
        ...
        [HttpGet]
        public async Task<ActionResult<IEnumerable<CarAdviceGrDictionaryMaterialQ>>> CarAdviceGrDictionaryMaterialQ()
        {
            if (_context.CarAdviceGrDictionaryMaterialQ == null)
            {
                return NotFound();
            }
            try
            {
                return await _context.CarAdviceGrDictionaryMaterialQ.ToListAsync();
            }
            catch (Exception ex)
            {
		...
		[HttpGet("{id}")]
		public async Task<ActionResult<CarAdviceGrDictionaryMaterialQ>> CarAdviceGrDictionaryMaterialQ(long id)
		{
			if (_context.CarAdviceGrDictionaryMaterialQ == null)
			{
				return NotFound();
			}
			var carAdviceGrDictionaryMaterialQ = await _context.CarAdviceGrDictionaryMaterialQ.FindAsync(id);

			if (carAdviceGrDictionaryMaterialQ == null)
			{
				return NotFound();
			}
